...
void
gdk_qnxscreen_surface_post_screen (GdkSurface *surface, int buffer_idx, int num_rects, cairo_rectangle_int_t *damaged_rects)
{
...
gdk_qnxscreen_surface_create_buffers (GdkQnxScreenSurface *qnx_screen_surface, int *buf_size)
{
  ...
  /* Set the buffer size */
  ret = screen_set_window_property_iv (qnx_screen_surface->window_handle, SCREEN_PROPERTY_BUFFER_SIZE, buf_size);
  if (ret < 0)
    {
      g_critical (G_STRLOC ": screen_set_buffer_property_iv(SCREEN_PROPERTY_BUFFER_SIZE): %s", strerror (errno));
      return ret;
    }
  ...
  surface->width = buf_size[0];
  surface->height = buf_size[1];
  ...
  // Reset GL surface
  gdk_surface_set_egl_native_window (surface, qnx_screen_surface->window_handle);
  ...
  if (width >= 0 && height >= 0 && width != impl->win_width && height != impl->win_height)
    {
      ...
      if (width > impl->buf_width || height > impl->buf_height)
        ...
        {
          /* We can keep the buffers, but we need to inform QNX Screen
           * of the new buffer size.
           * Just setting the "source size" is insufficient for GL coordiantes
           * to be properly reset. */
          ret = screen_set_window_property_iv (impl->window_handle, SCREEN_PROPERTY_BUFFER_SIZE, new_size);
          if (ret < 0)
            {
              g_critical (G_STRLOC ": screen_set_window_property_iv(SCREEN_PROPERTY_BUFFER_SIZE): %s", strerror (errno));
            }
          ...
          /* This tells GDK to destroy the current GL surface and recreate */
...
static void
gdk_qnxscreen_surface_class_init (GdkQnxScreenSurfaceClass *class)
{
...
gdk_qnxscreen_create_window (GdkQnxScreenSurface *qnx_screen_surface, int qnxscreen_window_type)
{
  ...
  const int usage = SCREEN_USAGE_WRITE | SCREEN_USAGE_NATIVE | SCREEN_USAGE_OPENGL_ES2 | SCREEN_USAGE_OPENGL_ES3; // TODO: GLES / Vulkan versions?
  ...
  GDK_DEBUG (MISC, "%s window position: %d,%d (default)",
             QNX_SCREEN, qnx_screen_surface->win_x, qnx_screen_surface->win_y);
  ...
  /* set screen swap interval */
  ret = screen_set_window_property_iv (qnx_screen_surface->window_handle, SCREEN_PROPERTY_SWAP_INTERVAL, &swap_interval);
  if (ret < 0)
    {
      g_critical (G_STRLOC ": failed to set window swap interval property: %s", strerror (errno));
      return;
    }
...
static void
gdk_qnxscreen_popup_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
gdk_qnxscreen_popup_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
show_grabbing_popup (GdkSeat *seat, GdkSurface *surface, gpointer user_data)
{
  show_popup (surface);
}

static gboolean
gdk_qnxscreen_surface_present_popup (GdkPopup *popup, int width, int height, GdkPopupLayout *layout)
{
...
static GdkGravity
gdk_qnxscreen_popup_get_surface_anchor (GdkPopup *popup)
{
  GdkSurface *surface = GDK_SURFACE (popup);
...
static GdkGravity
gdk_qnxscreen_popup_get_rect_anchor (GdkPopup *popup)
{
  GdkSurface *surface = GDK_SURFACE (popup);
...
static int
gdk_qnxscreen_popup_get_position_x (GdkPopup *popup)
{
  GdkSurface *surface = GDK_SURFACE (popup);
...
static int
gdk_qnxscreen_popup_get_position_y (GdkPopup *popup)
{
  GdkSurface *surface = GDK_SURFACE (popup);
...
static void
gdk_qnxscreen_popup_init (GdkQnxScreenPopup *popup)
{
  GDK_DEBUG (MISC, "%s initializing GdkQnxScreenPopup: %p", QNX_SCREEN, popup);
}

static void
gdk_qnxscreen_popup_constructed (GObject *object)
{
  GdkQnxScreenSurface *qnx_screen_surface = GDK_QNXSCREEN_SURFACE (object);
  GdkSurface *surface = GDK_SURFACE (qnx_screen_surface);
...
static void
gdk_qnxscreen_popup_class_init (GdkQnxScreenPopupClass *class)
{
...
void
gdk_qnxscreen_popup_iface_init (GdkPopupInterface *iface)
{
